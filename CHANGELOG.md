...
- **CVE-2023-45142**: Fixed unbounded cardinality in HTTP metrics that could lead to memory exhaustion.
  The `http.request.method` attribute in metrics generated by `go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp`
  is now restricted to a set of well-known HTTP methods. Unknown methods are replaced with `_OTHER`.
  High cardinality attributes `http.user_agent`, `http.client_ip`, `net.sock.peer.addr`, `net.sock.peer.port`,
  and `enduser.id` have been removed from metrics to prevent potential DoS attacks.
